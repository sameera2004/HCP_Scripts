...
StudyFolder="Datadir" #Location of Subject folders (named by subjectID)
Subjlist="Final_subject" #Space delimited list of subject IDs
EnvironmentScript="${XHOME}/projects/Pipelines/Examples/Scripts/SetUpHCPPipeline.sh" #Pipeline environment script
...
for Subject in $Subjlist ; do
  ...
  if [ -n "${command_line_specified_run_local}" ] ; then
      ...
      queuing_command="${FSLDIR}/bin/fsl_sub -j 13311 ${QUEUE}"
  fi
