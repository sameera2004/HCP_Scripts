...
set scanTypes = ( B0_fieldmap_Phase  B0_fieldmap B0_fieldmap_Magnitude Topup_AP Topup_PA SiemensFieldMap )
foreach y (`echo $scanTypes`)

	if ( $y == B0_fieldmap_Phase ) then 
		set scanname = GradientEchoFieldMap_Phase
        else if (  $y == B0_fieldmap ) then
		set scanname = GradientEchoFieldMap
        else if ( $y == SiemensFieldMap ) then
                set scanname = SiemensFieldMap
	else if ( $y == B0_fieldmap_Magnitude ) then 
		set scanname = GradientEchoFieldMap_Magnitude
...
#                echo "mri_convert -rt cubic -it dicom --out_orientation RAS ${dcm1} ${outdir}/${subjid}_3T_${scanname}_${mapnum}.nii.gz" >! scripts/mri_convert_${scanname}_${mapnum}.sh 
		...
		if ( $y == Topup_AP ||  $y == Topup_PA ) then
                        cp $niipath ${outdir}/${subjid}_3T_${scanname}_${mapnum}.nii
			pigz_mricron -f ${outdir}/${subjid}_3T_${scanname}_${mapnum}.nii
                        rm $niipath
                else if ( $y == B0_fieldmap_Phase || $y == B0_fieldmap  || $y == B0_fieldmap_Magnitude || $y == SiemensFieldMap) then
