...
for ((i=1;i<=${s_num};i++)); do
	...
	csh HCP_fMRI_prep.csh ${params_file}
        # if there are two sessions
        if [ ${s_num} -eq 2 ]; then
            # move the first session T1w and T2w to temporary folder
            if [ ${i} -eq 1 ]; then
            	sh T1w_T2w_mv_tmp.sh
            else 
                # renumber the first session T1w and T2w and move
                # data back into the subject/unprocessed/3T folder
