...
foreach y (`echo $scanTypes`)
	...
	foreach currscan (`echo $labelList`)
		...
		if ($currscan =~ *"SBRef") then 
		    ...
		    set outdir =  ${subjdir}/unprocessed/3T/2_${tempname}
		else
		    set outdir =  ${subjdir}/unprocessed/3T/2_${currscan}		
		endif		
		...
		mv $niipath ${outdir}/${subjid}_3T_2_${currscan}.nii
		pigz_mricron -f ${outdir}/${subjid}_3T_2_${currscan}.nii
		#rm niitmp >& tmp

                set topupscan = ${topupList[$x]}
                foreach s ( AP PA )
			rm ${outdir}/${subjid}_3T_SpinEchoFieldMap_${s}.nii.gz >& tmp
                        ln -s ${subjdir}/unprocessed/3T/FieldMap/2_${subjid}_3T_SpinEchoFieldMap_${s}_${topupscan}.nii.gz ${outdir}/${subjid}_3T_SpinEchoFieldMap_${s}.nii.gz
                end
